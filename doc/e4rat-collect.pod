...
B<e4rat-collect>  B<[> option(s) B<]>  B<[> application names B<]>
...
collect while executing command. e4rat-collect stops running after <command> terminates. Be aware that <command> gets executed with root privileges.
